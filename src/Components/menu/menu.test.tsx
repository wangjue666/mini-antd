...
describe('test Menu and MenuItem component', () => {
    ...
    it('should show dropdown items when hover on subMenu', async ()=>{
        ...
        await waitFor(() => {
            expect(wrapper.queryByText('dropdown')).toBeVisible(); // 显示出来
        });

        // 显示出来后，点击下拉菜单第一个item
        fireEvent.click(wrapper.getByText('drop1')); // 点击这个item， 触发 onSelect 事件
        expect(testProps.onSelect).toHaveBeenCalledWith('3-0'); // 返回参数 是不是  3-0 
        // 鼠标离开
        fireEvent.mouseLeave(dropdownElement); 
        await waitFor(() => {
            expect(wrapper.queryByText('drop1')).not.toBeVisible(); // 消失
        });
    })
})